...
- **版本更新**: 版本号升级至 12.11.5，同步更新安装脚本默认版本
- **子模块更新**: 更新 jstudio 子模块指针
</result
...
### 新功能
- **jstudio 文件预览**: `j read` 迁移为启动独立的 jstudio Tauri 桌面应用，替代旧的内置 reader Web 预览方案。
- **jstudio 子模块集成**: 新增 `apps/jstudio` 子模块，并提供初始化、开发、构建、安装、卸载、清理、状态查看与推送相关 Makefile 命令。
- **TUI 编辑器括号粘贴**: 编辑器支持终端 bracketed paste 事件，改善多行内容粘贴体验。

### 改进
- **文件预览文档**: 更新 `j read` 帮助文档，补充 jstudio 安装方式、应用查找顺序和常用构建命令说明。
- **TUI 显示宽度**: 修正非 ASCII 字符显示宽度计算，并统一交互指针样式为 `❯`。
- **命令保存 UI**: 优化命令保存级别选择界面，改善项目级目录不存在时的选择与创建逻辑。
- **stdin 读取处理**: 优化标准输入读取失败时的处理逻辑，提升异常场景稳定性。
- **依赖清理**: 移除旧 reader/server 方案不再使用的依赖，精简项目依赖集。

### Bug 修复
- **编辑器尾部渲染**: 修复 Markdown 编辑器滚动到底部时内容显示不全、底部提前出现 `~` 的问题。
- **块渲染裁剪**: 修复 Markdown 渲染态下块内容尾部被裁剪的问题，确保可视区域内内容完整渲染。
- **编辑工具提示**: 优化编辑工具渲染提示，修复部分提示显示不准确的问题。

# v12.10.107

### 改进

- **Terminal 主题显示**: 调整工具确认框、Ask 交互区和权限/Plan 确认区配色，改用主题定义的 reset/dark_gray 系列颜色，降低高饱和蓝/青色背景带来的突兀感。
- **弹窗选中态**: 统一 Chat 输入弹窗、命令弹窗、文件/技能/Slash 弹窗、Todo/Notebook 共享命令面板以及编辑器命令/主题弹窗的选中行样式，避免选中行内说明文字、标签或次要信息继续显示为 dim 色。
- **配置页选中行可读性**: 优化 Chat 配置页中 Commands、Skills、Teammates、全局配置行、开关行、预览行等列表项的选中态显示，使描述、标签、角色和调用次数等次要信息跟随选中前景色。
- **鼠标选区高亮**: Chat 消息区与配置页的鼠标文本选区改为使用主题的 popup highlight 样式，替代固定白字深灰背景，提升不同主题下的一致性。
- **Todo 渲染配色**: TodoWrite 请求和 Todo 结果渲染改用主题文本色与加载色，减少硬编码颜色并改善 terminal 主题下的可读性。
- **Reader Markdown 编辑体验**: 点击已渲染 Markdown 块时会根据点击位置定位到更准确的源码行和列，提升块级编辑时的光标定位体验。

### Bug 修复

- **Markdown 换行显示**: Reader 中 Markdown soft break 现在渲染为换行，修复软换行被显示为空格导致内容排版不符合预期的问题。
- **Reader 编辑器焦点样式**: 移除代码语言输入框和表格单元格编辑时额外的聚焦背景/阴影，避免选中或编辑状态下出现不一致的高亮效果。

# v12.10.106

### 新功能

- **Make 目标补全**: Shell 补全新增对 `make`/`gmake` 的 Makefile 目标解析，可在命令参数位置补全可见的 make target。
- **Markdown 表格可视化编辑**: Reader 的 Markdown 编辑器支持将表格块切换为单元格编辑模式，并自动保留/生成 Markdown 表格分隔行与对齐信息。
- **聊天文本主题配置**: 主题配置新增 `user_text`、`input_text`、`popup_highlight_bg`、`popup_highlight_fg` 等字段，支持独立控制用户消息、输入区和弹窗高亮样式。

### 改进

- **代码块编辑体验**: Reader 中 Markdown 代码块编辑器改为贴近渲染态的代码块样式，语言输入与代码内容编辑更统一。
- **弹窗高亮样式**: 命令弹窗、模型/归档列表和聊天补全弹窗统一使用主题化高亮，并支持反色高亮效果。
- **Markdown 主题细节**: 多个内置主题调整了列表符号颜色、AI 气泡背景和弹窗高亮配色，提升整体视觉一致性。

### Bug 修复

- **反色主题文本渲染**: 修复 terminal/reverse 主题下用户消息和输入区文本被气泡背景覆盖，导致反色显示异常的问题。
- **Markdown 编辑光标定位**: 修复表格/代码块等嵌套编辑区域中根据文档行号恢复光标时，起始行定位不准确的问题。

# v12.10.105

### 新功能

- **Shell 会话复用**: `! <command>` 与未匹配的交互命令现在会复用持久 Shell PTY 会话执行，支持保留 alias、环境初始化和命令间工作目录状态。
- **Shell 命令补全**: 交互模式补全支持从 `PATH` 读取可执行命令、从 zsh/bash 读取 alias，并为 `make`/`gmake` 补全 Makefile 目标。
- **Markdown 混合编辑**: Reader Markdown 编辑器改为点击块后仅当前块切换为源码编辑，其它块保持渲染态，并支持表格单元格可视化编辑。
- **分组使用技巧**: 使用技巧改为 YAML 分组维护，按 shell、prompt、completion、chat、alias 等功能域组织展示。
- **聊天文本主题配置**: 主题新增用户消息正文、输入区正文和弹窗高亮配色字段，支持独立配置聊天文本显示效果。

### 改进

- **交互 Prompt**: jcli REPL 和 shell prompt 增加当前目录、Git 分支及 dirty 标记展示，并优化命令输出与下一次输入之间的间距。
- **交互 Shell 体验**: Shell 命令执行改为 PTY 代理模式，支持方向键、Tab、Ctrl 组合键等终端输入，并在执行后同步子 Shell 当前目录。
- **Reader 编辑体验**: Markdown 列表、代码块、表格和空行编辑交互重构，减少 contenteditable 导致的内容错乱和光标跳转。
- **主题高亮表现**: 弹窗、命令列表和模型列表选中态改用统一高亮样式，终端主题支持反色高亮。

### Bug 修复

- **反色主题文本渲染**: 修复 terminal/reverse 主题下用户消息和输入区文本背景被气泡色覆盖，导致反色显示异常的问题。
- **交互式终端输入转发**: 修复交互式 shell 中方向键、控制键、组合键等原始输入转发不完整的问题。
- **交互式终端尺寸同步**: 启动和执行 PTY 命令时同步当前终端尺寸，修复显示换行和尺寸固定异常。
- **持久 Shell 状态恢复**: 修复命令完成标记读取、reader 恢复和 cwd 同步不可靠导致的交互 shell 代理状态异常。
- **Tips YAML 解析**: 修复 YAML 分组技巧提示缩进解析不准确的问题，并保留旧格式回退解析。
- **Reader Tab 关闭**: 修复关闭当前 Tab 后激活项选择异常的问题，现在优先切换到左侧相邻 Tab。

# v12.10.104

### Bug 修复

- **交互式 Shell 输入转发**: 改为直接从 stdin 读取原始字节并转发到 PTY，修复方向键、控制键、组合键等终端输入在交互式 shell 中转发不完整或异常的问题。
- **交互式 Shell 终端尺寸同步**: 启动 PTY 时使用当前终端尺寸，并在命令执行和输入循环中同步 resize，修复交互式 shell 尺寸固定导致的显示和换行异常。
...
### 新功能
- **Linux 发布包**: Release 流程新增 Linux x64 与 Linux ARM64 构建产物，并生成对应 tar.gz 包和 sha256 校验文件。
- **Reader 文件树**: 支持在文件树中通过右键菜单新建文件或目录。
- **Reader 主题切换**: Reader 增加主题切换能力，提升不同阅读场景下的显示适配。
- **终端背景主题**: 新增终端背景主题配置，支持更丰富的终端视觉样式。
- **Markdown 块编辑**: 增强 Markdown 块间编辑与块级编辑模式，改善文档编辑体验。

### 改进
- **Reader 设置与布局**: 优化 Reader 设置入口、布局结构和界面组织，让阅读与配置操作更清晰。
- **Reader 代码编辑**: 增强代码块编辑体验，优化代码内容展示与编辑交互。
- **Reader 列表与目录样式**: 调整 Reader 列表、目录和页面样式，提升整体可读性与导航体验。
- **README 内容结构**: 优化 README 内容组织，使项目信息和使用说明更易浏览。

### Bug 修复
- **代码块渲染**: 修复代码块中出现多余空行的问题。
- **引用块渲染**: 修正 Markdown 引用块显示异常。
- **列表项编辑**: 修复列表项在编辑状态下的渲染问题。
- **Tab 激活状态**: 修复关闭 Tab 后激活项异常的问题。

# v12.10.100

### 新功能

- **终端背景主题**: 新增 `terminal` 主题选项，可在主题列表与主题切换循环中选择“Terminal（跟随终端背景）”。

### Bug 修复

- **引用块渲染**: 修正引用块内容渲染宽度，渲染内部内容时会扣除引用前缀宽度，避免引用块内文本换行和布局异常。

# v12.10.99


### 新功能
- **Linux 发布包**: 发布流程新增 Linux x64 与 Linux ARM64 构建、打包和 SHA256 校验文件生成，支持在 GitHub Release 中提供 Linux 安装产物。
- **Reader 文件操作**: Reader 文件树支持右键新建，提升目录浏览与文件管理效率。
- **Reader 主题切换**: Reader 增加主题切换能力，支持在不同阅读风格之间切换。
- **Reader 代码高亮**: Reader 前端引入更完整的代码高亮构建产物，提升 Markdown 代码块和代码文件的阅读体验。

### 改进
- **Reader 设置与布局**: 优化 Reader 设置入口、工具箱展示和编辑区布局，改善阅读与编辑体验。
- **Reader 界面样式**: 优化文件树、Tab、目录和 Markdown 文档样式，使界面层次更清晰。
- **README**: 重组功能介绍、安装说明和截图内容，改善项目文档的可读性。

### Bug 修复
- **代码块渲染**: 修复 Reader 代码块中出现多余空行的问题。
...
- **Reader 活动栏**: 新增 VSCode 风格的左侧活动栏，支持在文件树和工具箱之间快速切换（⌘1 / ⌘2 快捷键），选中状态持久化到 localStorage
- **Reader 工具箱**: 新增工具箱面板，集中管理内置小工具，点击即可作为独立 Tab 打开
- **文本 Diff 工具**: 新增纯前端文本差异比较工具，支持左右双栏输入、行级差异高亮（LCS 算法）、交换/清空操作，内容通过 sessionStorage 持久化
- **JSON 查看器工具**: 新增 JSON 格式化与树形查看工具，支持节点折叠/展开、叶子值编辑、格式化/压缩切换、复制输出
- **图片查看器**: 新增只读图片查看器，通过 `/api/asset` 加载原始图片，底部展示分辨率、文件大小和 MIME 信息
- **纯文本编辑器**: 新增非受控 `textarea` 纯文本编辑器，避免高频按键触发 React 整体重渲染，大文件编辑体验更流畅

### 改进

- **Reader 前端架构重构**: 将 Reader/Remote 前端代码独立至 `assets/reader` 目录，引入完整的前端工程化配置（ESLint、Prettier、package.json），构建产物输出到 `assets/reader/dist`
- **Reader 侧栏可调节宽度**: 侧栏宽度支持拖拽调节（180px ~ 560px），宽度值持久化到 localStorage
- **Reader 退出确认弹窗**: 新增关闭未保存文件时的二次确认弹窗（保存/不保存/取消）和退出 Reader 的确认弹窗，防止误操作丢失内容
- **Reader 目录入口支持**: Reader 支持目录级打开（不预选文件），仅展示文件树供用户浏览选择
- **Reader 心跳机制**: 新增 5 秒间隔心跳请求，服务端 30 秒未收到即自动 shutdown，覆盖浏览器异常关闭场景

### Bug 修复

- **Hinter 提示被误补全**: 引入 `DisplayOnlyHint` 类型，使 Hinter 展示的使用技巧和历史提示不会被右箭头键自动补全到输入行
</result
...
- **笔记本顶部栏样式简化**: 将顶部栏从圆角全边框改为仅底部分隔线，配色调整为 DarkGray，视觉更轻量；同时根据当前模式动态调整顶部栏高度（Normal/Adding/Renaming 为 2 行，其他带输入框模式为 3 行），节省垂直空间。
- **笔记本编辑区空提示居中**: 编辑区无内容时的"选择笔记以编辑内容"提示文字改为水平和垂直双向居中显示，视觉更协调。

### 内部优化

- **布局计算统一为单一事实来源**: 提取 `NotebookLayout` 结构体和 `compute_layout()` 函数，UI 渲染与鼠标事件命中检测共享同一份布局计算逻辑，消除了 handler 中独立硬编码的 `MouseLayoutInfo`，避免因顶栏高度等变更导致 UI 与鼠标命中区域漂移不一致的问题。
