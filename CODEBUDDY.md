...
The `SubCmd` enum in `src/cli.rs` defines all subcommands with their clap derive attributes.

### Config (`src/config/yaml_config.rs`)

All configuration is stored in `~/.jdata/config.yaml` and deserialized into `YamlConfig` via serde. Contains alias maps, category tags, and runtime settings. Passed as `&mut YamlConfig` throughout the command layer.

### Command Layer (`src/command/`)

- `mod.rs` — `dispatch()` function routing `SubCmd` variants to handlers; contains keyword/category constants
- `open.rs` — Core alias resolution: looks up alias in config, determines if path/URL/script/app, spawns system process
...
Fullscreen multi-line editor built on `ratatui` + `tui-textarea` (patched local copy in `patches/tui-textarea-0.7.0/`). Used for writing reports, scripts, and composing multi-line content.
...
- `md_render.rs` — Native Rust Markdown terminal rendering via crossterm ANSI output, reusing the chat module's Markdown parser
- `fuzzy.rs` — Simple fuzzy string matching used by `search` command

### Embedded Assets (`src/assets.rs`, `assets/`)

`assets/help.md` and `assets/version.md` are embedded at compile time via `include_str!`. AI default prompts (`system_prompt_default.md`, `memory_default.md`, `soul_default.md`) are also embedded.
