...
pub fn chat_oneshot(
    ...
    if agent_config.providers.is_empty() {
        return Err("尚未配置 AI Provider，请先在终端运行 `j chat` 进行配置".to_string());
    }
    ...
    let result = rt.block_on(async {
        api::call_openai_simple(provider, &send_messages, load_system_prompt().as_deref()).await
    });
    ...
    match result {
        Ok(response_text) => {
            if !response_text.is_empty() {
                ...
                persist_messages(
                    &session_id,
                    &[ChatMessage::text("assistant", &response_text)],
                    0,
                );
                Ok(response_text)
            } else {
                Err("AI 返回空响应".to_string())
            }
        }
        Err(e) => Err(format!("API 调用失败: {}", e)),
    }
}
