...
/// GUI 用：获取待办列表
pub fn list_todos() -> Result<Vec<String>, String> {
    let todo_list = load_todo_list();
    let items: Vec<String> = todo_list
        .items
        .iter()
        .map(|item| {
            if item.done {
...
/// GUI 用：添加待办
pub fn add_todo(content: &str) -> Result<String, String> {
    let mut todo_list = load_todo_list();
    todo_list.items.push(TodoItem {
    ...
    if save_todo_list(&todo_list) {
        ...
        Err("保存待办失败".to_string())
    }
}

/// GUI 用：完成待办（1-indexed）
pub fn complete_todo(index: usize) -> Result<String, String> {
    let mut todo_list = load_todo_list();
    if index == 0 || index > todo_list.items.len() {
        return Err(format!("索引 {} 超出范围", index));
    }

    let item = &mut todo_list.items[index - 1];
    if item.done {
        return Ok(format!("待办 {} 已经完成", index));
    }
    ...
    if save_todo_list(&todo_list) {
        Ok(format!("✅ 已完成待办: {}", content))
    } else {
        Err("保存待办失败".to_string())
    }
}

/// GUI 用：删除待办（1-indexed）
pub fn remove_todo(index: usize) -> Result<String, String> {
    let mut todo_list = load_todo_list();
    if index == 0 || index > todo_list.items.len() {
        return Err(format!("索引 {} 超出范围", index));
    }

    let removed = todo_list.items.remove(index - 1);

    if save_todo_list(&todo_list) {
        Ok(format!("✅ 已删除待办: {}", removed.content))
    } else {
        Err("保存待办失败".to_string())
    }
}
