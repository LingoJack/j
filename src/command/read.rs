...
fn launch_reader(target: &Path) -> Result<(), String> {
    if let Some(reader) = reader_from_env() {
        return spawn_reader_bin(&reader, target);
    }
    ...
    if let Some(bin) = first_existing_path(WINDOWS_BIN_PATHS) {
        return spawn_reader_bin(&bin, target);
    }
    ...
    Err(reader_missing_message())
}

fn reader_from_env() -> Option<PathBuf> {
    std::env::var_os(READER_BIN_ENV)
        .map(PathBuf::from)
...
fn spawn_reader_bin(bin: &Path, target: &Path) -> Result<(), String> {
    Command::new(bin)
        .arg(target)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .map_err(|e| format!("无法启动 jstudio reader：{e}"))?;
    Ok(())
}
