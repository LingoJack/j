...
pub fn handle_concat(name: &str, content: &[String], config: &mut YamlConfig) {
    output_result(&handle_concat_with_result(name, content, config));
}

/// 处理 concat 命令（返回结果版本）
pub fn handle_concat_with_result(name: &str, content: &[String], config: &mut YamlConfig) -> CommandResult {
    ...
    if config.contains(section::PATH, name) {
            ...
            .or_else(|| config.get_property(section::PATH, name))
        {
            Some(p) => p.clone(),
            None => {
                return CommandResult::error(format!("❌ 别名 {{{}}} 已存在，但未找到对应的脚本路径", name));
            }
        };
        ...
        let existing_content = match fs::read_to_string(&existing_path) {
            Ok(c) => c,
            Err(e) => {
                return CommandResult::error(format!("❌ 读取已有脚本文件失败: {} (路径: {})", e, existing_path));
            }
        };
        ...
        match crate::tui::editor::open_multiline_editor_with_content(
            ...
            Ok(Some(new_content)) => {
                if new_content.trim().is_empty() {
                    return CommandResult::error("⚠️ 脚本内容为空，未保存修改");
                }
                // 写回脚本文件
                match fs::write(&existing_path, &new_content) {
                    Ok(_) => CommandResult::with_output(format!("✅ 脚本 {{{}}} 已更新，路径: {}", name, existing_path)),
                    Err(e) => CommandResult::error(format!("💥 写入脚本文件失败: {}", e)),
                }
            }
            Ok(None) => {
                CommandResult::with_output("已取消编辑脚本")
            }
            Err(e) => {
        ...
        let script_content = if content.is_empty() {
            ...
            match crate::tui::editor::open_multiline_editor_with_content(
                ...
                Ok(Some(text)) => text,
                Ok(None) => {
                    return CommandResult::with_output("已取消创建脚本");
                }
                Err(e) => {
                    return CommandResult::error(format!("❌ 编辑器启动失败: {}", e));
                }
            }
        } else {
            // 有内容参数：拼接并去除两端引号
        ...
        if script_content.trim().is_empty() {
            return CommandResult::error("⚠️ 脚本内容为空，无法创建");
        }
        ...
        if let Some(parent) = script_path.parent() {
            if let Err(e) = fs::create_dir_all(parent) {
                return CommandResult::error(format!("❌ 创建目录失败: {}", e));
            }
        }
        ...
        match fs::write(&script_path, &script_content) {
            ...
            Err(e) => {
                return CommandResult::error(format!("💥 写入脚本文件失败: {}", e));
            }
        }
