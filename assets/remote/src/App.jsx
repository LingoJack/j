...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] max-w-[680px] mx-auto">
      {/* Header */}
      <div className="flex items-center gap-3 px-5 pt-[max(20px,env(safe-area-inset-top))] pb-4 bg-bg2/95 backdrop-blur-sm border-b border-border shrink-0">
        <div className={`w-3 h-3 rounded-full shrink-0 transition-colors duration-300 ${connected ? 'bg-ok shadow-[0_0_8px_var(--color-ok)]' : 'bg-fg3'}`} />
        <span className="text-[22px]">🦞</span>
        <span className="font-bold text-[19px] tracking-wide">Sprite</span>
        <span className="text-border-light mx-1 text-lg">|</span>
        <span className="text-label-ai text-[15px] font-medium">{modelName}</span>
        <span className="ml-auto text-fg2 text-sm font-medium">📬 {msgCount}</span>
      </div>

      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3 [-webkit-overflow-scrolling:touch]" ref={messagesRef}>
        {messages.length === 0 && (
      ...
      {/* Status Bar */}
      <div className={`px-5 py-2 text-center text-[13px] bg-bg2/95 backdrop-blur-sm border-t border-border shrink-0 flex items-center justify-center gap-2 ${isLoading ? 'text-warn' : 'text-fg3'}`}>
        {isLoading && <span className="w-2 h-2 rounded-full bg-warn animate-[pulse_1.2s_ease-in-out_infinite]" />}
        {statusText}
      </div>

      {/* Input Area */}
      <div className="flex gap-3 items-end px-4 pt-3.5 pb-[max(16px,env(safe-area-inset-bottom))] bg-bg2/95 backdrop-blur-sm border-t border-border shrink-0">
        <textarea
          ref={textareaRef}
          rows={1}
          placeholder="输入消息..."
        ...
        {isLoading ? (
          <button
            className="w-[52px] h-[52px] rounded-full border-none text-lg cursor-pointer flex items-center justify-center shrink-0 transition-all duration-150 bg-danger text-white active:scale-[0.92]"
            onClick={cancelStream}
            title="取消"
          >■</button>
        ) : (
          <button
            className="w-[52px] h-[52px] rounded-full border-none text-2xl cursor-pointer flex items-center justify-center shrink-0 transition-all duration-150 bg-label-user text-white disabled:opacity-30 disabled:cursor-default enabled:active:scale-[0.92]"
            onClick={sendMessage}
