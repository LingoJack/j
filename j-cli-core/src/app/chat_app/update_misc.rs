use super::ChatApp;
use crate::app::action::CursorDirection;
use crate::app::types::ToolExecStatus;
use crate::app::ui_state::ChatMode;
use crate::constants::{FINE_SCROLL_LINES, INPUT_BUFFER_MAX_LEN, PAGE_SCROLL_LINES};
use crate::protocol::{ToolConfirmInfo, WsOutbound};
use crate::storage::{MessageRole, save_agent_config};
...
impl ChatApp {
    ...
    pub(super) fn update_stream_error(&mut self, e: &crate::chat_error::ChatError) {
        self.state.retry_hint = None;
    ...
    pub(super) fn update_browse_copy_message(&mut self) {
        use crate::render::cache::copy_to_clipboard;
        let copy_result = {
    ...
    pub(super) fn update_cancel_stream(&mut self) {
        ...
        if let Some(req) = self.ui.pending_plan_approval.take() {
            req.resolve(crate::app::types::PlanDecision::Reject);
        }
        if matches!(
            self.ui.mode,
    ...
    pub(super) fn update_toggle_auto_approve(&mut self) {
        ...
        if let Some(mut meta) = crate::storage::load_session_meta_file(&sid) {
            meta.auto_approve = self.ui.auto_approve;
            crate::storage::save_session_meta_file(&meta);
        }
        self.show_toast(
