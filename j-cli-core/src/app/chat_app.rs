...
use crate::agent_md;
use crate::constants::TODO_NAG_INTERVAL_ROUNDS;
use crate::context::message_compress::{
    DEFAULT_OTHER_AGENT_TOOLCALL_THRESHOLD, compress_other_agent_toolcalls,
};
use crate::infra::command::{self, CommandSource};
use crate::infra::hook::{HookContext, HookEvent, HookManager, HookResult};
use crate::infra::sandbox::Sandbox;
use crate::infra::skill;
...
use crate::tools::background::{BackgroundManager, build_running_summary};
use crate::tools::derived_shared::{
    AgentContextConfig, DerivedAgentShared, SubAgentTracker,
};
use crate::tools::plan::PlanApprovalQueue;
use crate::tools::task::{TaskManager, build_tasks_summary};
...
pub struct ChatApp {
    ...
    /// 远程控制 WebSocket 桥接器
    pub ws_bridge: Option<crate::remote::bridge::WsBridge>,
    /// 远程客户端是否已连接
    ...
    pub invoked_skills: crate::context::compact::InvokedSkillsMap,
}
...
impl ChatApp {
    ...
    pub fn new(session_id: String) -> Self {
        ...
        let invoked_skills = crate::context::compact::new_invoked_skills_map();
        let mut tool_registry = ToolRegistry::new(
            ...
            crate::storage::SessionPaths::new(&session_id).todos_file(),
        );
        ...
        let agent_context_config = Arc::new(Mutex::new(
            crate::tools::derived_shared::AgentContextConfig {
        ...
        let shared_sub_agent_metrics = Arc::new(Mutex::new(
            crate::tools::derived_shared::SubAgentMetrics::default(),
        ));
        ...
        if let Ok(mut manager) = hook_manager.lock() {
            ...
            manager.register_builtin(HookEvent::PreLlmRequest, "broadcast_compress", |ctx| {
                ...
                let self_name = crate::agent::thread_identity::current_agent_name();
            ...
            ui: UIState {
                ...
                quote_idx: {
                    let ms = std::time::SystemTime::now()
                        ...
                        .unwrap_or_default()
                        .as_millis() as usize;
                    ms % crate::ui::quotes::quotes_count()
                },
                input_wrap_width: 0,
    ...
    pub fn build_sync_outbound(&self) -> WsOutbound {
        use crate::protocol::{SyncMessage, SyncToolCall};
    ...
    pub fn broadcast_config_state(&mut self) {
        use crate::protocol::{ConfigField, ModelInfo, ThemeInfo};
    ...
    pub fn broadcast_archive_list_state(&self) {
        use crate::protocol::ArchiveInfo;
    ...
    pub fn handle_file_list(path: &str) -> Vec<crate::protocol::FileEntry> {
...
mod tests {
    use super::ChatApp;
    use crate::app::types::{ToolCallStatus, ToolExecStatus};
    use crate::app::ui_state::ChatMode;
