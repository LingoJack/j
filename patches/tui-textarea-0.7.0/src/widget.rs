...
impl<'a> TextArea<'a> {
    ...
    fn scroll_top_col(&self, prev_top: u16, width: u16) -> u16 {
        // 使用显示宽度（而非字符索引）来计算光标的实际列位置
        // 这修复了中文等双宽字符导致水平滚动延迟的问题
        let (row, col) = self.cursor();
        let line = self.lines().get(row).map(|l| l.as_str()).unwrap_or("");
        let display_col: u16 = line.chars()
            .take(col)
            .map(|c| if c.is_ascii() { 1u16 } else { 2u16 })
            .sum();
        let mut cursor = display_col;
