...
export function ResultList() {
  const { results, selectedIndex, setSelectedIndex, outputMode, textOutput } = useSearchStore();
  ...
  // 文本输出模式
  if (outputMode === 'text' && textOutput) {
    return (
      <div className="border-t border-white/[0.06]">
        <div className="py-3 px-4 max-h-[280px] overflow-y-auto result-scroll">
          <pre className="text-[13px] text-text-secondary whitespace-pre-wrap font-mono leading-relaxed">
            {textOutput}
          </pre>
        </div>
      </div>
  ...
  // 列表输出模式
  if (outputMode === 'list' && results.length > 0) {
    return (
      <div className="border-t border-white/[0.06]">
